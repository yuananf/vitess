...
// SplitStatementToSlice returns the splitted sql statements
func SplitStatementToSlice(blob string) (queries []string, err error) {
	...
	for {
		...
		if tkn == ';' {
			...
			stmt = blob[stmtBegin:]
			if len(stmt) > 0 && strings.TrimSpace(stmt) != "" {
				queries = append(queries, stmt)
			}
			break
		}
